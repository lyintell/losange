...
export default function ChantierStatutBadge({ status, onStatusChange, disabled = false }) {
  const bgColor = getChantierStatusColor(status);
  ...
  const handlePress = () => {
    ...
    pressTimerRef.current = setTimeout(() => {
      ...
      if (pressCount >= 2) {
        onStatusChange?.(getNextChantierStatusOnDoubleTap(status));
      }
